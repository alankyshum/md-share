...
//   - <br/> and <br> tags become newlines (markmap renders them as line breaks).
...
function stripShape(raw: string): string {
  let s = raw.trim();
  // Strip optional leading id (alphanumeric followed by a shape opener)
  // e.g. "id1((text))" → "((text))"
  ...
  // Convert <br/> / <br> to space — markmap doesn't natively understand them in plain text
...
export function mermaidMindmapToMarkdown(source: string): string | null {
  ...
  // Collect non-empty lines + their indent
  ...
  // Normalise depths: smallest indent → 0, next → 1, …
  ...
  // Build a nested-list markdown representation.
  // Treat the shallowest level as the root — emit it as an H1 so markmap uses it
  // as the diagram's centre node. Everything deeper becomes a list with 2-space
  // indent per extra level.
  const out: string[] = [];
  ...
  for (const { level, text } of normalised) {
    if (level === 0 && !rootEmitted) {
      ...
      // Multiple roots — keep them as h2 children of an implicit root
      out.push(`## ${text}`);
    } else {
