...
export function lintMarkdown(md: string): string[] {
  ...
  if (unbalancedAt !== null) errors.push(`L${unbalancedAt}: unclosed fenced code block (missing closing \`\`\`)`);
  ...
  for (const v of BUILTIN_FENCE_VALIDATORS) {
    ...
    for (const l of langs) fenceMap.set(l, v);
  }

  for (const block of blocks) {
    const v = fenceMap.get(block.lang);
    if (!v) continue;
    if (v.validate) errors.push(...v.validate(block.body, { startLine: block.startLine }));
  }

  for (const dv of BUILTIN_DOC_VALIDATORS) errors.push(...dv.validate(md));
  return errors;
}
